...
import { ArrowLeft, CheckCircle2, MapPin, Ticket } from "lucide-vue-next"
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
import { hasRequiredContactFields, updateTextField } from "@/composables/useContactFields"
import { festivalInfo } from "@/data/festival"
import { useDayProgram } from "./useDayProgram"
...
const { completeProgram, day, dayTitle, timelineItems } = useDayProgram(() => props.dayId)
const canSubmitTicket = computed(() => hasRequiredContactFields(ticketForm))
...
function updateTicketField(field: TicketFormField, event: Event) {
  updateTextField(ticketForm, field, event)
...
<template>
  <section class="border-b border-foreground bg-background">
    <div class="mx-auto w-full max-w-368 border-x border-foreground">
      ...
      >
        <section class="min-h-96 border-b border-foreground lg:min-h-136 lg:border-b-0 lg:border-r" aria-label="Mapa de ubicación">
          <iframe
            :src="venueMapUrl"
            title="Mapa de Las Naves, Valencia"
            class="h-full min-h-96 w-full lg:min-h-136"
            loading="lazy"
            allowfullscreen

...
const activityBlocks = [
  ...
  },
  {
    id: "tienda",
    title: "TIENDA",
    space: "Tienda online · merch oficial Esclat",
    summary: "Merchandising oficial del festival: ropa, bolsas, accesorios y objetos diseñados para llevarte un pedazo de Esclat contigo.",
  ...
  <section class="border-b border-foreground bg-background">
    ...
    >
      ...
      >
        ...
        <p class="mt-3 w-full text-[0.82rem] leading-relaxed text-foreground/90 sm:mt-4 sm:text-base lg:text-lg">
          {{ block.summary }}
        </p>
        ...
        <div class="grid min-w-full border-b border-grey" :style="scheduleTableStyle">
          ...
          <div v-for="row in activeScheduleRows" :key="row.id" class="grid border-b border-grey last:border-b-0" :style="scheduleRowGridStyle">
            <div class="sticky left-0 z-20 flex flex-col justify-center gap-1 border-r border-grey bg-background px-2 py-2.5 text-left sm:px-4 sm:py-4">
              <span class="wrap-break-word text-[0.66rem] font-semibold uppercase leading-tight tracking-normal text-black sm:whitespace-nowrap sm:text-sm">{{ row.label }}</span>
              <span class="wrap-break-word text-[0.48rem] font-medium uppercase leading-tight tracking-wide text-foreground/50 sm:whitespace-nowrap sm:text-[0.68rem]">{{ row.sublabel }}</span>
            </div>
